...
def vote():
    ...
    try:
        ...
        if voter_id:
            if save_voter_to_database(voter_id, token, cursor):
                if authenticate_face():
                    vote = vote_for_party()
                    if vote is not None:
                        update_vote_in_database(token, vote, cursor)
                        return jsonify({"message": "Vote recorded successfully"}), 200
                    else:
                        return jsonify({"error": "Invalid party choice"}), 400
                else:
                    return jsonify({"error": "Authentication failed"}), 400
            else:
                return jsonify({"error": "Failed to save voter details"}), 400
        else:
            return jsonify({"error": "Voter ID not found"}), 400
